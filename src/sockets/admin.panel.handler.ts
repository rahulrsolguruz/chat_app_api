...
export const userManagementEventHandler = async (socket) => {
  const admin_id = socket.admin?.id;
  await db.insert(user_activity).values({
    admin_id: admin_id,
    target_id: admin_id,
    target_type: 'admin',
    activity_type: ENUM.ActivityType.ADMIN_CONNECTED
  });
  // Check if the user is an admin and join the admin room

  socket.join('admins');
  logger.info(`Admin with ID ${admin_id} joined the admin room`);
...
export const groupManagementEventHandler = async (socket) => {
  const admin_id = socket.admin?.id;
  socket.join('admins');
  logger.info(`Admin with ID ${admin_id} joined the admin room`);
  ...
  socket.on(EVENTS.ADMIN.GROUP_CREATED, async (data) => {
    ...
    // Emit event to notify clients
    socket.to('admins').emit(EVENTS.ADMIN.GROUP_CREATED, { group_id, group_name, group_picture_url, group_admin });
  ...
  socket.on(EVENTS.ADMIN.GROUP_UPDATED, async (data) => {
    ...
    // Emit event to notify clients
    socket.to('admins').emit(EVENTS.ADMIN.GROUP_UPDATED, { group_id, group_name });
  });
  ...
  socket.on(EVENTS.ADMIN.GROUP_MEMBER_ADDED, async (data) => {
    ...
    // Emit event to notify clients
    socket.to('admins').emit(EVENTS.ADMIN.GROUP_MEMBER_ADDED, { group_id, user_id, role });
  });
  ...
  socket.on(EVENTS.ADMIN.GROUP_MEMBER_REMOVED, async (data) => {
    ...
    // Emit event to notify clients
    socket.to('admins').emit(EVENTS.ADMIN.GROUP_MEMBER_REMOVED, { group_id, user_id, role });
  });
};
